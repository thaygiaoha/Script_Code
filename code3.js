...
function getFirebaseConfigByIdgv(idgv) {
  ...
  try {
    if (typeof ssAdmin === "undefined" || !ssAdmin) return defaultConfig;
    var sheet = ssAdmin.getSheetByName("idgv");
    if (!sheet) return defaultConfig;
    var lastRow = sheet.getLastRow();
    if (lastRow < 2) return defaultConfig;

    // Lấy đến cột M (cột 13)
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (!cellA) continue;
      var colAStr = String(cellA).replace(/'/g, "").trim().toUpperCase();
      if (colAStr === targetStr) {
        var rawColM = String(data[i][12] || "").trim(); // Cột M (Index 12)
        if (rawColM) {
          try {
            // Nếu lưu dạng JSON object
            var parsed = JSON.parse(rawColM);
            return parsed;
          } catch(e) {
            // Nếu chỉ lưu tên projectId
            return {
              projectId: rawColM,
              apiKey: defaultConfig.apiKey,
              authDomain: rawColM + ".firebaseapp.com"
            };
          }
        }
      }
    ...
    Logger.log("Lỗi getFirebaseConfigByIdgv: " + err.toString());
  }
...
if (action === 'getLG') {
  try {
    ...
    const passTracuu = String(params.pass || "").trim();
    const idgvTracuu = String(params.idgv || "").trim();
...
// ==========================================================
// HỆ THỐNG ĐỒNG BỘ 2 CHIỀU GOOGLE APPS SCRIPT <-> FIREBASE FIRESTORE
// ==========================================================

/**
 * Ghi 1 document lên Firebase Firestore qua REST API
 */
function setFirestoreDoc_(projectId, collectionName, docId, dataObj) {
  if (!projectId || !collectionName || !docId) return false;
  try {
    var url = "https://firestore.googleapis.com/v1/projects/" + projectId + "/databases/(default)/documents/" + collectionName + "/" + encodeURIComponent(docId);
    
    // Chuyển JavaScript object sang Firestore fields format
    var fields = {};
    ...
    Logger.log("Lỗi setFirestoreDoc_: " + err.toString());
    return false;
  }
}

/**
 * Đồng bộ danh sách học sinh từ Sheet của GV lên Firebase
 */
function syncStudentsToFirebase(idgv, sheetId) {
  try {
    var ssTarget = getSS2(sheetId, idgv);
    var sheetHS = ssTarget.getSheetByName("hocsinh") || ssTarget.getSheetByName("danhsach");
    ...
    if (values.length < 2) return { status: "success", count: 0 };

    var count = 0;
    for (var i = 1; i < values.length; i++) {
      var row = values[i];
      var sbd = String(row[0] || "").trim().toUpperCase();
      var pass = String(row[1] || "").trim();
      var name = String(row[2] || "").trim();
      var lop = String(row[3] || "").trim();
      var limittab = row[4] !== undefined ? row[4] : 3;
      var limit = row[5] !== undefined ? row[5] : 1;
      ...
      if (sbd) {
        ...
        // Ghi vào doc: teachers/{idgv}/students/{sbd}
        setFirestoreDoc_(projectId, "teachers/" + cleanIdgv + "/students", sbd, studentData);
        count++;
      }
    }

    return { status: "success", count: count, message: "Đã đồng bộ " + count + " học sinh lên Firebase!" };
  } catch (err) {
